...
def lbp_descriptor(src):
    ...
    """
    # pick middle element as p
    p = src[1,1]

    binary_image = src.copy()
    ...
    powers_of_two = np.array([[1, 2, 4], [2**7, 0, 2**3], [2**6, 2**5, 2**4]])

    res = np.sum(np.multiply(powers_of_two, binary_image))
...
def lbp_feature(src):
    ...
    padded_image = np.pad(gray, 1, mode='constant')
    lbf_image = gray.copy()

    # create lbf image
    for i in range(1, rows):
      for j in range(1, cols):
        lbf_region = padded_image[i-1:i+2, j-1:j+2]
        lbf_image[i][j] = lbp_descriptor(lbf_region)
    ...
    while(True):
        ...
        for (column, row, width, height) in detected_faces:
            ...
            if ismatch:
                ...
                cv2.rectangle(frame,(column, row),(column + width, row + height),(255, 0, 0),2)
